...
def login_process(ip: str, host_local: str, device_info: list):
...
if __name__ == '__main__':
    ...
    with ProcessPoolExecutor(max_workers=4) as executor:
        login_func = partial(login_process, ip, config["global"]["host_local"])
        executor.map(login_func, ip_config["info_list"])
